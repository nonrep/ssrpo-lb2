...
class AppNameGenerator : public Generator_interface
{
public:
...
class CategoryGenerator : public Generator_interface {
...
public:
    ...
    virtual std::string generate() const override {
        int random_index = std::rand() % categories.size(); // Случайное число в диапазоне [0, 9]
        return categories[random_index]; // Выбираем случайную категорию
    }
};

class PriceGenerator : public Generator_interface {
public:
    virtual std::string generate() const override {
        return std::to_string(std::rand() % 10); // Случайное число от 0 до 100 долларов
    }
};

class SizeGenerator : public Generator_interface
{
public:
    virtual std::string generate() const override 
    { 
        return std::to_string(std::rand()%501); 
    }
};

class InstallationsGenerator : public Generator_interface {
public:
    virtual std::string generate() const override {
        return std::to_string(std::rand() % 10001); // Случайное число от 0 до 10,000 установок
    }
};

class RatingGenerator : public Generator_interface {
public:
    virtual std::string generate() const override {
        return std::to_string((std::rand() % 5) + 1); // Случайное число от 1 до 5
    }
};
...
std::string generateLine(std::string str, int line_no, const std::string & control_character)
{
    std::vector<std::pair<std::string,std::shared_ptr<Generator_interface>>> replaces
    {
        ...
        {control_character+"N", std::make_shared<AppNameGenerator>()},
        {control_character+"C", std::make_shared<CategoryGenerator>()},
        {control_character+"P", std::make_shared<PriceGenerator>()},
        {control_character+"S", std::make_shared<SizeGenerator>()},
        {control_character+"D", std::make_shared<InstallationsGenerator>()},
        {control_character+"R", std::make_shared<RatingGenerator>()},
